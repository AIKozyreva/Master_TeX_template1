...
\newfontfamily\cyrillicfont{Times New Roman} % Определяем моноширинный шрифт для кириллицы
...
\hypersetup{
    ...
    pdfauthor={Анфиса Козырева},
    pdftitle={Сравнение АФЗ-систем Vch по геномным и протеомным данным}
}
...
% Списки
\usepackage{enumitem}
\setlist[enumerate,itemize]{leftmargin=1.25mm} % Отступы в списках
...
\titleformat{\subsection}[block]{\bfseries\vspace{0.5em}}{}{12.5mm}{\thesubsection\quad#1}
...
% Подсекции без номеров (НЕ добавляются в оглавление)
\newcommand{\anonsubsection}[1]{%
    \vspace{1.5em}%                  % Отступ перед заголовком
    %\noindent%                       % Убираем абзацный отступ
    \hangindent=1.25mm%              % Отступ слева как у subsection
    {#1}
    \par%              % Завершаем абзац с заголовком
    \vspace{0.5em}%                  % Отступ после заголовка
}
...
% Секция для задания ВКР (она не включается в содержание)
